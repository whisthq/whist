...
// agent interacting with the database rows for the containers once the
...
func VerifyAllocatedContainer(userID fctypes.UserID, containerID fctypes.FractalID) error {
	...
	rows, err := q.FindContainersForUser(context.Background(), string(instanceName), string(userID))
	if err != nil {
