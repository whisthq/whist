...
func (c *containerData) WriteLocalDevValues(protocolTimeout int) error {
	// Write timeout
	if err := c.writeResourceMappingToFile("timeout", utils.Sprintf("%v", protocolTimeout)); err != nil {
		// Don't need to wrap err here because it already contains the relevant info
		return err
	}
	return nil
...
func (c *containerData) getResourceMappingDir() string {
	return utils.Sprintf("%s%s/containerResourceMappings/", utils.FractalDir, c.fractalID)
}
