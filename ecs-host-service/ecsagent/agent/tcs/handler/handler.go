...
import (
	...
	tcsclient "github.com/fractal/fractal/ecs-host-service/ecsagent/agent/tcs/client"
...
func StartSession(params *TelemetrySessionParams, statsEngine stats.Engine) error {
	...
	for {
		...
		if tcsError == nil || tcsError == io.EOF {
			...
			// fractal-specific change: make this a warning to stop spamming sentry
			seelog.Warnf("Error: lost websocket connection with ECS Telemetry service (TCS): %v", tcsError)
...
func startSession(
	...
	err = client.Connect()
	if err != nil {
		// fractal-specific change: we make this a warning to reduce sentry spam
		seelog.Warnf("Error connecting to TCS: %v", err.Error())
		return err
	}
