...
type DockerTaskEngine struct {
	...
	// Fractal-specific change: keep track of host service goroutines
	fractalGoroutineTracker *sync.WaitGroup
...
func (engine *DockerTaskEngine) Init(ctx context.Context, fractalGoroutineTracker *sync.WaitGroup) error {
	derivedCtx, cancel := context.WithCancel(ctx)
	engine.stopEngine = cancel
	engine.ctx = derivedCtx
	engine.fractalGoroutineTracker = fractalGoroutineTracker
...
func (engine *DockerTaskEngine) MustInit(ctx context.Context, fractalGoroutineTracker *sync.WaitGroup) {
	...
	retry.RetryWithBackoff(taskEngineConnectBackoff, func() error {
		...
		err := engine.Init(ctx, fractalGoroutineTracker)
		if err != nil {
			errorOnce.Do(func() {
...
func (engine *DockerTaskEngine) createContainer(task *apitask.Task, container *apicontainer.Container) dockerapi.DockerContainerMetadata {
	...
	seelog.Infof("Container VolumesFrom: %v", container.VolumesFrom)
	seelog.Infof("Container MountPoints: %v", container.MountPoints)
	seelog.Infof("Container VolumesUnsafe: %v", container.VolumesUnsafe)
	seelog.Infof("Container Ports: %v", container.Ports)
	seelog.Infof("Container KnownPortBindings: %v", container.GetKnownPortBindings())

	// Fractal-Specific change: we set the FractalID for this container (for an
	...
	// If the creation of the container fails, we want to clean up after it.
	var createFailed bool = true
	defer func() {
		if createFailed {
			container.FractalContainer.Close()
		}
	}()
	...
	// Fractal-Specific change: we request port bindings for the container from
	// the host service. We do this before we resolve the hostConfig so that it
	// reflects the changes we made to `container`.
	seelog.Infof("Requesting port bindings from host service for FractalID %s", container.FractalContainer.GetFractalID())
	perr := container.FractalContainer.AssignPortBindings(container.Ports)
	if perr != nil {
		herr := &apierrors.HostConfigError{
			Msg: fmt.Sprintf("AssignPortBindings for FractalID %s returned an error: %s", fractalID, perr),
		}
		return dockerapi.DockerContainerMetadata{Error: apierrors.NamedError(herr)}
	}
	container.Ports = container.FractalContainer.GetPortBindings()
	...
	// Fractal-Specific change: we request uinput devices from the higher layers
	// of the host service and inject them into the container.
	seelog.Infof("Requesting uinput devices from host service for FractalID %s", fractalID)
	deverr := container.FractalContainer.InitializeUinputDevices(engine.fractalGoroutineTracker)
	if deverr != nil {
		herr := &apierrors.HostConfigError{
			Msg: fmt.Sprintf("InitializeUinputDevices() for FractalID %s returned an error: %s", fractalID, deverr),
		}
		return dockerapi.DockerContainerMetadata{Error: apierrors.NamedError(herr)}
	}
	hostConfig.Resources.Devices = append(hostConfig.Resources.Devices, container.FractalContainer.GetDeviceMappings()...)
	seelog.Infof("Appended fractal devices to hostConfig for container with FractalID %s. New hostConfig.Resources.Devices: %v", fractalID, hostConfig.Resources.Devices)

	// Fractal-specific change: we allocate a TTY for the container.
	seelog.Infof("Requesting tty from the host service for FractalID %s", fractalID)
	ttyerr := container.FractalContainer.InitializeTTY()
	if ttyerr != nil {
		herr := &apierrors.HostConfigError{
			Msg: fmt.Sprintf("InitializeTTY() for FractalID %s returned an error: %s", fractalID, ttyerr),
		}
		return dockerapi.DockerContainerMetadata{Error: apierrors.NamedError(herr)}
	}
	...
	seelog.Infof("task.DockerConfig returned config: %v", config)
	...
	if metadata.DockerID != "" {
		...
		// Fractal-specific change: we need to write the created resources' values
		// into the container.
		if err := container.FractalContainer.WriteResourcesForProtocol(); err != nil {
			// Don't need to wrap `err.Error()` here, since it contains `fractalID`
			herr := &apierrors.HostConfigError{Msg: err.Error()}
			seelog.Error(herr)
			return dockerapi.DockerContainerMetadata{Error: apierrors.NamedError(herr)}
		...
		seelog.Errorf("Got back a blank metadata from Docker for container with FractalID %s", fractalID)
	}
	...
	// Prevent FractalContainer object from being closed, since container
