...
import (
	...
	"github.com/fractal/fractal/ecs-host-service/ecsagent/agent/taskresource/envFiles"
	"github.com/fractal/fractal/ecs-host-service/ecsagent/agent/taskresource/firelens"
	"github.com/fractal/fractal/ecs-host-service/ecsagent/agent/taskresource/ssmsecret"
	...
	taskresourcevolume "github.com/fractal/fractal/ecs-host-service/ecsagent/agent/taskresource/volume"
...
func (task *Task) dockerExposedPorts(container *apicontainer.Container) nat.PortSet {
	...
	for _, portBinding := range container.Ports {
		dockerPort := nat.Port(strconv.Itoa(int(portBinding.ContainerPort)) + "/" + string(portBinding.Protocol))
...
func (task *Task) dockerHostBinds(container *apicontainer.Container) ([]string, error) {
	...
	seelog.Infof("Task.dockerHostBinds(): pre-modification container.MountPoints: %v", container.MountPoints)
	...
	for i, mountPoint := range container.MountPoints {
		...
		// Fractal-specific change: we replace a certain string, denoted
		// FRACTAL_ID_PLACEHOLDER, in the directories passed to the internal docker
	...
	seelog.Infof("Task.dockerHostBinds(): returned binds: %v", binds)
