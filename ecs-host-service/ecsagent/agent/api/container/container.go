...
type Container struct {
	...
	RuntimeID string
	// FractalContainer contains the Fractal-specific metadata (see the
	// ecs-host-service for more details), including the FractalID.
	FractalContainer fractalcontainer.FractalContainer
	...
	// TODO(djsavvy,fractal): get rid of this, to avoid duplication of state between the FractalContainer and this field
	Ports []fractalportbindings.PortBinding `json:"portMappings"`
	...
	KnownPortBindingsUnsafe []fractalportbindings.PortBinding `json:"KnownPortBindings"`
...
func (c *Container) SetKnownPortBindings(ports []fractalportbindings.PortBinding) {
	c.lock.Lock()
	defer c.lock.Unlock()
...
func (c *Container) GetKnownPortBindings() []fractalportbindings.PortBinding {
	c.lock.RLock()
	defer c.lock.RUnlock()
