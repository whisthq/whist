...
def swapSpecificDisk(self, disk_name, vm_name):
    ...
    print(
        f"SUCCESS: Disk swapped out in {end - start:0.4f} seconds. Restarting " + vm_name)
...
def storeLogs(self, sender, connection_id, logs, vm_ip):
    title = '[{}] Logs Received From Connect #{}'.format(
        logs, str(connection_id))
    ...
    if sender.upper() == 'CLIENT':
        ...
        with engine.connect() as conn:
            ...
            title = '[{}] Logs Received From Connection #{}'.format(
                sender.upper(), str(connection_id))

            command = text("""
                SELECT * FROM logs WHERE "connection_id" = :connection_id
                """)
            params = {'connection_id': connection_id}
            logs_found = cleanFetchedSQL(
            ...
            if logs_found:
                ...
                params = {'username': username, 'ip': ip, 'last_updated': last_updated,
                          'logs': logs, 'connection_id': connection_id}
