...
def swapDisk(self, disk_name, ID=-1):
    ...
    if vm_attached:
        vm_name = vm_name.split('/')[-1]
        sendInfo(ID, 'Disk {} already attached to VM {}'.format(
            disk_name, vm_name))

        self.update_state(state='PENDING', meta={
                          "msg": "Server is updating. Waiting for update to finish."})
        hr = spinLock(vm_name)

        if hr > 0:
            self.update_state(state='PENDING', meta={
                              "msg": "Data already uploaded to server. Updating database."})
            ...
            self.update_state(state='PENDING', meta={
                              "msg": "Database updated. Booting Cloud PC."})

            sendInfo(ID, 'Database updated with {} and {}'.format(
                disk_name, vm_name))

            if fractalVMStart(vm_name) > 0:
                sendInfo(ID, 'VM {} is started and ready to use'.format(vm_name))
                self.update_state(state='PENDING', meta={
                                  "msg": "Cloud PC is ready to use."})
            else:
    ...
    if not vm_attached:
        ...
        while not free_vm_found:
            ...
            if available_vms:
                self.update_state(state='PENDING', meta={
                                  "msg": "Uploading your data to an available server. This could take a few minutes."})
                sendInfo(ID, 'Found {} available VMs'.format(
                    str(len(available_vms))))
                # Pick a VM, attach it to disk
                vm_name = available_vms[0]['vm_name']
                lockVM(vm_name, True, ID=ID)
                updateVMState(vm_name, 'ATTACHING')
                sendInfo(ID, 'Selected VM {} to attach to disk {}'.format(
                    vm_name, disk_name))
                if swapDiskAndUpdate(self, disk_name, vm_name) > 0:
                    self.update_state(state='PENDING', meta={
                                      "msg": "Data successfully uploaded to cloud PC."})
                    free_vm_found = True
                ...
                sendInfo(
                    ID, 'Could not find a running and available VM to attach to disk {}'.format(disk_name))
                deactivated_vms = fetchAttachableVMs(
                    'DEALLOCATED', location) + fetchAttachableVMs('STOPPED', location)
                if deactivated_vms:
                    self.update_state(state='PENDING', meta={
                                      "msg": "Uploading your data to a hibernating server. This could take a few minutes."})
                    ...
                    self.update_state(state='PENDING', meta={
                                      "msg": "All servers are in currently use. Waiting for one to become available."})
