        ...
        if sender == "CLIENT":
            ...
            if logs_found:
                command = text(
                    """
					UPDATE logs 
					SET "last_updated" = :last_updated, "server_logs" = :file_name, "ip" = :vm_ip, "version" = :version, "username" = :username
					WHERE "connection_id" = :connection_id
					"""
                )
                params = {
                ...
                command = text(
                    """
					INSERT INTO logs("last_updated", "server_logs", "connection_id", "ip", "version", "username") 
					VALUES(:last_updated, :file_name, :connection_id, :vm_ip, :version, :username)
					"""
                )
...
def deleteLogsInS3(connection_id, ID=-1):
    ...
    with engine.connect() as conn:
        ...
        if logs_found["server_logs"]:
            success_serverlogs = S3Delete(logs_found["server_logs"], last_updated, ID)
        ...
        if logs_found["client_logs"]:
            ...
            success_clientlogs = S3Delete(logs_found["client_logs"], last_updated, ID)
