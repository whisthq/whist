...
def test_cluster_with_auto_scaling_group():
    ...
    cluster_name, launch_config_name = testclient.create_launch_configuration(
        instance_type="t2.small", ami="ami-026f9e275180a6982"
    )
    auto_scaling_group_name = testclient.create_auto_scaling_group(
        launch_config_name=launch_config_name
    )
    auto_scaling_groups_def = testclient.auto_scaling_client.describe_auto_scaling_groups(
    ...
    capacity_provider_name = testclient.create_capacity_provider(
        auto_scaling_group_name=auto_scaling_group_name
    )
    capacity_providers_def = testclient.ecs_client.describe_capacity_providers(
    ...
    assert (
        capacity_provider["autoScalingGroupProvider"]["autoScalingGroupArn"]
    ...
    assert testclient.task_ips[0] in testclient.get_container_instance_ips(
    ...
    command_id = testclient.exec_commands_on_containers(
        cluster_name, container_instances, ["echo hello"]
    )["Command"]["CommandId"]
