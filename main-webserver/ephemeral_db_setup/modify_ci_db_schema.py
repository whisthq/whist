...
def change_schema_for_ci():
    ...
    given by Heroku in CI.

    Needs db_schema.sql in the current directory and env vars POSTGRES_REMOTE_URI
    and POSTGRES_LOCAL_URI.
    """
    assert "db_schema.sql" in os.listdir(".")
    postgres_remote_uri = os.getenv("POSTGRES_REMOTE_URI", "")
    postgres_local_uri = os.getenv("POSTGRES_LOCAL_URI", "")
    assert postgres_remote_uri != "" and postgres_local_uri != ""
    prefix = "postgres://"
    assert postgres_remote_uri[: len(prefix)] == prefix
    assert postgres_local_uri[: len(prefix)] == prefix
    role, db = parse_role_and_db_from_url(postgres_remote_uri)
    local_role, local_db = parse_role_and_db_from_url(postgres_local_uri)
