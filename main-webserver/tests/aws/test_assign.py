...
def test_bad_app(client, authorized):
    response = client.post("/container/assign", json=dict(username=authorized, app="Bad App"))
...
def test_no_app(client, authorized):
    response = client.post("/container/assign", json=dict(username=authorized))

    assert response.status_code == HTTPStatus.BAD_REQUEST


def test_no_region(client, authorized):
    response = client.post("/container/assign", json=dict(username=authorized, app="VSCode"))
