...
audio_device_t *CreateAudioDevice() {
  ...
  res =
      snd_pcm_open(&audio_device->handle, "default", SND_PCM_STREAM_CAPTURE, 0);
  if (res < 0) {
  ...
  res = snd_pcm_hw_params_set_period_size_near(
      audio_device->handle, audio_device->params, &audio_device->num_frames, 0);
