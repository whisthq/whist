    ...
    if (updating_set_clipboard) {
      ...
      */
      ...
      if (GetTimer(clipboard_time) < spam_time_ms / 1000.0) {
        SDL_Delay(
            max((int)(spam_time_ms - 1000 * GetTimer(clipboard_time)), 1));
      }
    }
...
ClipboardData* ClipboardSynchronizerGetNewClipboard() {
  ...
  // If the clipboard has updated since we last checked, or a previous clipboard
  // update is still pending, then we try to update the clipboard
  if (hasClipboardUpdated() || pendingUpdateClipboard()) {
    if (updating_clipboard) {
      // Clipboard is busy, to set pending update clipboard to true to make sure
      // we keep checking the clipboard state
      pending_update_clipboard = true;
    } else {
