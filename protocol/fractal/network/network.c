...
int SendUDPPacket(SocketContext *context, FractalPacketType type, void *data,
  ...
  double num_indices_per_unit_latency =
      (AVERAGE_LATENCY_MS / 1000.0) * (burst_bitrate / 8.0) / MAX_PAYLOAD_SIZE;
...
FractalPacket *ReadTCPPacket(SocketContext *context) {
  ...
  if ((unsigned long)reading_packet_len > sizeof(int)) {
    ...
    if (target_len >= 0 && target_len <= LARGEST_TCP_PACKET &&
      ...
      for (unsigned long i = start_next_bytes; i < sizeof(int) + actual_len;
           i++) {
...
int CreateTCPServerContext(SocketContext *context, int port,
  ...
  // Bind to port
  if (bind(context->s, (struct sockaddr *)(&origin_addr), sizeof(origin_addr)) <
      0) {
...
int CreateUDPServerContextStun(SocketContext *context, int port,
  ...
  while ((recv_size = recvfrom(context->s, (char *)&entry, sizeof(entry), 0,
    ...
    if (GetTimer(recv_timer) * 1000 < stun_timeout_ms &&
      ...
      if (sendto(context->s, (const char *)&stun_request, sizeof(stun_request),
                 0, (struct sockaddr *)&stun_addr, sizeof(stun_addr)) < 0) {
        LOG_WARNING("Could not send message to STUN %d\n",
                    GetLastNetworkError());
        closesocket(context->s);
        return -1;
