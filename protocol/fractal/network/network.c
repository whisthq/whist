...
bool handshake_private_key(SocketContextData* context) {
  ...
  */
  ...
  while (
    (recv_size = recvfrom(context->socket, (char*)&their_priv_key_data, sizeof(their_priv_key_data),
  ...
  recv_size =
    recv(context->socket, (char*)&our_signed_priv_key_data, sizeof(our_signed_priv_key_data), 0);
