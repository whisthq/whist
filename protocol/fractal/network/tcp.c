    ...
    if (target_len >= 0 && actual_len >= target_len) {
      ...
      if (ENCRYPTING_PACKETS) {
        ...
        decrypted_len = decrypt_packet_n(
          ...
          decrypted_packet_buffer, target_len, (unsigned char*)context->binary_aes_private_key);
      } else {
...
int tcp_send_constructed_packet(void* raw_context, FractalPacket* packet, size_t packet_size) {
  ...
  int encrypted_len =
    encrypt_packet(packet, unencrypted_len, (FractalPacket*)(sizeof(int) + encrypted_packet_buffer),
...
bool create_tcp_socket_context(SocketContext* network_context, char* destination, int port,
  ...
  */
  ...
  if (using_stun) {
    if (destination == NULL)
      ret =
        create_tcp_server_context_stun(context, port, recvfrom_timeout_ms, connection_timeout_ms);
    else
