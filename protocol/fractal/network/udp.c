...
int udp_send_packet(void* raw_context, FractalPacketType packet_type, void* payload,
    ...
    if (nack_buffer && context->fec_packet_ratio > 0.0) {
        ...
        // a(1-c) = bc
        // a = bc/(1-c)
        num_fec_packets = num_indices * context->fec_packet_ratio / (1.0 - context->fec_packet_ratio);
    }
...
#define MAX_TOTAL_PACKETS 4096
    ...
    char* buffer_sizes[MAX_TOTAL_PACKETS];
    ...
    for (int packet_index = 0; packet_index < num_indices; packet_index++) {
        ...
        if (fec_encoder) {
            ...
            encoder_register_buffer(fec_encoder, (char*)payload + current_position, packet_payload_size);
        } else {
            // Populate the buffers directly when not using FEC
            buffers[i] = (char*)payload + current_position;
            buffer_sizes[i] = packet_payload_size;
        }
...
void udp_update_bitrate_settings(SocketContext* socket_context, int burst_bitrate,
    ...
    if (fec_packet_ratio > MAX_FEC_RATIO) {
        LOG_ERROR("%f is larger than the maximum FEC ratio!", fec_packet_ratio);
        fec_packet_ratio = MAX_FEC_RATIO;
    }
    if (fec_packet_ratio < 0.0) {
        LOG_ERROR("FEC ratio of %f, is negative!???", fec_packet_ratio);
        fec_packet_ratio = 0.0;
    }
