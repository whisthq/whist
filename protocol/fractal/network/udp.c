...
int udp_nack(SocketContext* socket_context, FractalPacketType type, int packet_id,
  ...
  FractalPacket* packet =
    &context
       ->nack_buffers[type_index][packet_id % context->nack_num_buffers[type_index]][packet_index];
...
int create_udp_server_context(void* raw_context, int port, int recvfrom_timeout_ms,
  ...
  int recv_size;
  if ((recv_size =
         recvfrom(context->socket, NULL, 0, 0, (struct sockaddr*)(&context->addr), &slen)) != 0) {
...
bool create_udp_socket_context(SocketContext* network_context, char* destination, int port,
  ...
  */
  ...
  if (using_stun) {
    if (destination == NULL)
      ret =
        create_udp_server_context_stun(context, port, recvfrom_timeout_ms, connection_timeout_ms);
    else
