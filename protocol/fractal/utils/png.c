...
int read_char_open( AVFormatContext** pctx, const char* data, int data_size )
{
    ...
    if( !infmt )
        infmt = av_find_input_format( "png" );
    *pctx = avformat_alloc_context();
    ...
    AVIOContext* pbctx = avio_alloc_context( buffer, data_size, 0, &opaque, read_open, NULL, NULL );
    (*pctx)->pb = pbctx;
...
int load_png( uint8_t* data[4], int linesize[4], unsigned int* w, unsigned int* h, enum AVPixelFormat* pix_fmt, char* png_filename )
{
    ...
    *pix_fmt = frame->format;
    ...
    struct SwsContext* swsContext = sws_getContext(*w, *h, *pix_fmt, *w, *h, AV_PIX_FMT_RGB24, 0, NULL, NULL, NULL );
