...
int bmp_to_png(char* bmp, int bmp_size, char** png, int* png_size) {
  ...
  */
  ...
  if (scanline_bytes * h != data_size) {
    LOG_WARNING("BMP scanline * h <> BMP header data size mismatch %d * %d != %d", scanline_bytes,
                h, data_size);
    return -1;
  }
  ...
  if (file_size < pixel_offset + data_size) {
    LOG_ERROR("BMP file_size is too small to fit PIXEL_OFFSET + DATA_SIZE, %d < %d + %d", file_size,
              pixel_offset, data_size);
  }
