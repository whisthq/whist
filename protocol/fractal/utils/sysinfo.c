...
void print_os_info() {
  ...
  struct utsname uts;
  uname(&uts);
  snprintf(buf, sizeof(buf), "%s %s %s %s %s", uts.machine, uts.sysname, uts.nodename, uts.release,
           uts.version);
  LOG_INFO("  OS: %s", buf);
#else
...
#define MAX_NUM_ADAPTERS 10
  ...
  for (i = 0; i < num_adapters; i++) {
    IDXGIOutput* output;
    for (j = 0; adapters[i]->lpVtbl->EnumOutputs(adapters[i], j, &output) != DXGI_ERROR_NOT_FOUND;
         j++) {
