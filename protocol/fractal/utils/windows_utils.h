...
This file contains all code that interacts directly with Windows desktops (Winlogon, the login screen, and regular desktops).
...
DesktopContext: This type represents a Windows desktop object.
    - This object can be used to represent a Windows desktop to set to (for instance, Winsta0, the default Windows desktop)
...
/*
@brief                          Open current or specified Desktop and set its information in DesktopContext, and set the current thread to the opened Desktop
...
/*
@brief                          Open a Windows window station (a handle to a Desktop session), like Winsta0 or Winlogon
*/
void OpenWindow();

/*
@brief                          Call the above functions; logs from Winlogon to Winsta0 (the standard Windows desktop) by entering the Fractal password
