...
void encoder_register_buffer(FECEncoder* fec_encoder_raw, char* buffer, int buffer_size) {
    ...
    if (buffer_size > fec_encoder->packet_size) {
        LOG_ERROR("This buffer is too large! %d instead of %d", buffer_size, fec_encoder->packet_size);
        return;
    }
...
FECDecoder* create_fec_decoder_context(int num_real_buffers, int num_total_buffers, int packet_size) {
...
void decoder_register_buffer(FECDecoder* fec_decoder_raw, int index, char* buffer,
    ...
    if (buffer_size > fec_decoder->packet_size) {
        LOG_ERROR("This buffer is too large! %d instead of %d", buffer_size, fec_decoder->packet_size);
        return;
    }
