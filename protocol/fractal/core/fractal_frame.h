 ...
 * the given frame. Pass NULL to embed no cursor whatsoever. Default of a 0'ed Frame* is already a
 * NULL cursor.
 */
 ...
 * pointer: frame->videodata_length must be set set_frame_cursor_image must be called After writing
 * to the returned PeerUpdateMessage*, please update frame->num_peer_update_msgs
 *
 * @param frame                    The Frame who's data buffer is being used
 *
 ...
 * the Frame's buffer. Even if the Frame* is being stored in a much larger buffer, this function
 * returns only the number of bytes needed for the data inside the Frame* to be read correctly.
