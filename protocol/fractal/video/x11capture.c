...
int CreateCaptureDevice(CaptureDevice* device, UINT width, UINT height) {
  ...
  device->damage =
      XDamageCreate(device->display, device->root, XDamageReportRawRectangles);
  ...
  device->segment.shmid =
      shmget(IPC_PRIVATE, device->image->bytes_per_line * device->image->height,
             IPC_CREAT | 0777);
...
#if USING_SHM
      if (!XShmGetImage(device->display, device->root, device->image, 0, 0,
                        AllPlanes)) {
