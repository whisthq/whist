...
void SetClipboard( ClipboardData* cb ) {
  ...
  // If it's text or an image, simply set the data and type to the current clipboard
  // so that the clipboard acts just like it did when the previous GetClipboard was
  // called
  //
  // If cb->type == CLIPBOARD_FILES, then we take all of the files and folers in
  // "./set_clipboard" and set them to be our clipboard. These files and folders
  // should not be symlinks, simply assume that they will never be symlinks
  //
  ...
  if (cb->type == CLIPBOARD_TEXT) {
    ...
    *((int*)(&file_buf[10])) = 54;
    ...
    inp = popen("xclip -i -sel clipboard -t x-special/gnome-copied-files", "w");
  ...
  // We make sure that pending clipboard updates are wiped out, because we just recently updated the clipboard
  // hasClipboardUpdated() is going to return true right now:
  hasClipboardUpdated();
