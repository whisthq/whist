...
void SetClipboard(ClipboardData* cb) {
  ...
  // We make sure that pending clipboard updates are wiped out, because we just
  // recently updated the clipboard hasClipboardUpdated() is going to return
  // true right now:
  hasClipboardUpdated();
...
bool get_clipboard_data(Atom property_atom, ClipboardData* cb,
  ...
  if (new_atom == incr_id) {
    ...
    do {
      ...
      XGetWindowProperty(display, window, property_atom, 0, LONG_MAX / 4, True,
                         AnyPropertyType, &new_atom, &resbits, &ressize,
