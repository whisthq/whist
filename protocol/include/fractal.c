...
int CreateTCPServerContextStun( struct SocketContext* context, char* destination, int port, int recvfrom_timeout_ms, int stun_timeout_ms ) {
    context->is_tcp = true;
...
int CreateTCPClientContext( struct SocketContext* context, char* destination, int port, int recvfrom_timeout_ms, int stun_timeout_ms ) {
    context->is_tcp = true;
...
int CreateTCPClientContextStun( struct SocketContext* context, char* destination, int port, int recvfrom_timeout_ms, int stun_timeout_ms ) {
    context->is_tcp = true;
...
int CreateTCPContext2( struct SocketContext* context, char* origin, char* destination, int port, int recvfrom_timeout_ms, int stun_timeout_ms) {
    ...
    if(origin[0] == 'C')
        ...
        return CreateTCPServerContextStun(context, destination, port, recvfrom_timeout_ms, stun_timeout_ms);
#else
    if(origin[0] == 'C')
            return CreateTCPClientContext(context, destination, port, recvfrom_timeout_ms, stun_timeout_ms);
        else
            return CreateTCPServerContext(context, destination, port, recvfrom_timeout_ms, stun_timeout_ms);
#endif
}


int CreateUDPServerContext(struct SocketContext* context, char* destination, int port, int recvfrom_timeout_ms, int stun_timeout_ms) {
    context->is_tcp = false;
    // Create UDP socket
    context->s = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
    if (context->s <= 0) { // Windows & Unix cases
    ...
    stun_entry_t entry = {0};
    int recv_size;
    while ((recv_size = recvfrom(context->s, (char *) &entry, sizeof(entry), 0, (struct sockaddr *) (&context->addr),
                                 &slen)) < 0) {
...
int CreateUDPServerContextStun(struct SocketContext* context, char* destination, int port, int recvfrom_timeout_ms, int stun_timeout_ms) {
    context->is_tcp = false;
    ...
    mprintf("Sending stun entry to STUN...\n");
    if (sendto(context->s, (const char *) &stun_request, sizeof(stun_request), 0, (struct sockaddr *) &stun_addr,
               sizeof(stun_addr)) < 0) {
        mprintf("Could not send message to STUN %d\n", GetLastNetworkError());
        closesocket(context->s);
        return -1;
    ...
    while ((recv_size = recvfrom(context->s, (char *) &entry, sizeof(entry), 0, (struct sockaddr *) (&context->addr),
        ...
        if (GetTimer(recv_timer) * 1000 < stun_timeout_ms &&
            (GetLastNetworkError() == ETIMEDOUT || GetLastNetworkError() == EAGAIN)) {
...
int CreateUDPClientContextStun(struct SocketContext* context, char* destination, int port, int recvfrom_timeout_ms, int stun_timeout_ms) {
    context->is_tcp = false;
...
int CreateUDPContext2( struct SocketContext* context, char* origin, char* destination, int port, int recvfrom_timeout_ms, int stun_timeout_ms) {
    ...
    if(origin[0] == 'C')
        ...
        return CreateUDPServerContextStun(context, destination, port, recvfrom_timeout_ms, stun_timeout_ms);
#else
    if(origin[0] == 'C')
            return CreateUDPClientContext(context, destination, port, recvfrom_timeout_ms, stun_timeout_ms);
        else
            return CreateUDPServerContext(context, destination, port, recvfrom_timeout_ms, stun_timeout_ms);
#endif
}
