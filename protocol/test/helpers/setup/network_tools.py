...
def setup_artificial_network_conditions(
    ...
    if network_conditions == "normal":
        ...
        command = f"( nohup timeout {testing_time+5}s ~/whist/protocol/test/helpers/apply_network_conditions.sh -d {','.join(network_devices)} {degradations_command} > ~/network_conditions.log 2>&1 & )"
        pexpect_process.sendline(command)
        wait_until_cmd_done(pexpect_process, pexpect_prompt, running_in_ci)
