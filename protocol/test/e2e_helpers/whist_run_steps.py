...
def build_mandelboxes_on_instance(
    ...
    command = f"cd ~/whist/mandelboxes && ./build.sh {mandelbox_name} --{cmake_build_type} --quiet | tee ~/{role}_mandelbox_build.log"
...
def run_mandelbox_on_instance(pexpect_process, role, json_data=None, simulate_scrolling=0):
    ...
    the browsers/whistium mandelbox has already been built. Further, the host service must be
