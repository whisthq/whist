    ...
    bool run_capture_test(CaptureDevice *capture, uint32_t width, uint32_t height,
        ...
        for (int i = 0; i < 20; i++) {
            ...
            if (ret > 0) {
                ...
                ret = transfer_screen(capture, &cuda_context);
                EXPECT_GE(ret, 0);
...
static bool storePicture(const char *target, uint32_t id, uint32_t width, uint32_t height,
                         void *frame_data, uint32_t stride) {
    char path[256];
    bool ret = false;
...
static bool testDecodeFrame(VideoEncoder *encoder, VideoDecoder *decoder, uint32_t width,
                            uint32_t height, bool dump) {
    ...
    if (dump) {
        ...
        return storePicture("/tmp/picture_%d.yuv", frame_id++, frame->width, frame->height,
                            frame->data[0], frame->linesize[0]);
    }
    return true;
}

static bool captureEncodeDecodeFrame(CaptureDevice *capture, uint32_t width, uint32_t height,
    ...
    EXPECT_TRUE(testDecodeFrame(encoder, decoder, width, height, true));
    return true;
}
...
static int32_t capture_and_encode_thread(void *opaque) {
    ...
    data->test_result &= captureEncodeDecodeFrame(&data->capture_device, data->width, data->height,
...
TEST_F(CaptureDeviceTest, CaptureAndEncodeNvX11) {
    ...
    uint32_t bitrate = 18857939;
    uint32_t vbv = 7543175;
    int i, ret;
    ...
    ret = create_capture_device(&data.capture_device, capture_type, (void *)xcontext.displayStr,
                                data.width, data.height, 81);
    EXPECT_GE(ret, 0);
    if (ret < 0) goto out;

    data.encoder = create_video_encoder(data.width, data.height, bitrate, vbv, CODEC_TYPE_H264);
