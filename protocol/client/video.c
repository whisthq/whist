    ...
    if ((FractalCursorID)cursor->cursor_id != last_cursor || cursor->using_bmp) {
      ...
      if (cursor->using_bmp) {
        ...
        SDL_Surface* cursor_surface =
          SDL_CreateRGBSurfaceFrom(cursor->bmp, cursor->bmp_width, cursor->bmp_height,
        ...
        sdl_cursor =
          SDL_CreateColorCursor(scaled_cursor_surface, cursor->bmp_hot_x * 96 / cursor_dpi,
...
void update_sws_context() {
  ...
  */
  ...
  video_context.sws =
    sws_getContext(decoder->width, decoder->height, sws_input_fmt, decoder->width, decoder->height,
  ...
  if (rendering) {
    ...
    while ((res = video_decoder_get_frame(video_context.decoder)) == 0) {
      ...
      if (render_this_frame) {
        ...
        int ret = SDL_UpdateNVTexture(video_context.texture, &texture_rect, video_context.data[0],
                                      video_context.linesize[0], video_context.data[1],
                                      video_context.linesize[1]);
        if (ret == -1) {
