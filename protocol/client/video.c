...
void update_video() {
    ...
    */
    ...
    static const int packet_size = sizeof(FractalPacket);
...
#if LOG_VIDEO
        LOG_INFO("bitrate for last %f ms: %f mbps (%d packets), client bitrate: %f mbps",
                 throughput_time * 1000, bitrate / (1024 * 1024),
                 video_ring_buffer->bytes_received / packet_size, client_bitrate / (1024 * 1024.0));
#endif
    ...
    if (get_timer(video_data.frame_timer) > 3) {
        ...
        // EXPERIMENTAL
        // We multiply by 1.5 because experimentally, we receive packets at a lower bitrate than specified.
        max_bitrate = client_bitrate * 1.5;
        update_mbps = true;
        start_timer(&video_data.frame_timer);
    }
