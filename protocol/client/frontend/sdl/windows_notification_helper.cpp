// This file, along with the matching header, is provided by Microsoft
// to support WRL notifications in desktop applications.
...
HRESULT CreateToastNotifier(IToastNotifier **notifier) {
    ...
    if (s_aumid.empty()) {
        ...
        return toastStatics->CreateToastNotifierWithId(HStringReference(s_aumid.c_str()).Get(),
                                                       notifier);
    }
}
...
bool CanUseHttpImages() { return IsRunningAsUwp(); }
...
HRESULT DesktopNotificationHistoryCompat::Remove(const wchar_t *tag) {
    if (m_aumid.empty()) {
        ...
        return m_history->RemoveGroupedTagWithId(HStringReference(tag).Get(),
                                                 HStringReference(L"").Get(),
                                                 HStringReference(m_aumid.c_str()).Get());
    }
}

HRESULT DesktopNotificationHistoryCompat::RemoveGroupedTag(const wchar_t *tag,
                                                           const wchar_t *group) {
    if (m_aumid.empty()) {
...
HRESULT DesktopNotificationHistoryCompat::RemoveGroup(const wchar_t *group) {
    if (m_aumid.empty()) {
        ...
        return m_history->RemoveGroupWithId(HStringReference(group).Get(),
                                            HStringReference(m_aumid.c_str()).Get());
    }
}
