...
void update_current_device(whist_server_state* state, clock* statistics_timer,
  ...
  LOG_INFO("Received an update capture device request to dimensions %dx%d with DPI %d", true_width,
           true_height, state->client_dpi);
      ...
      if (disable_encoder) {
        ...
        if (encoder->encoded_frame_size != 0) {
          if (encoder->encoded_frame_size > (int)MAX_VIDEOFRAME_DATA_SIZE) {
            ...
            send_populated_frames(state, &statistics_timer, &server_frame_timer, device, encoder,
                                  id);
