...
This file contains all code that interacts directly with processing audio
packets on the client.
...
decoder, and set the proper audio parameters for receiving from the server

*/
void initAudio();

/*
@brief                          Receives a FractalPacket into an audio packet,
and check if NACKing is necessary

@param packet                   Packet of data that gets received over a
SocketContext

@returns                        Returns -1 if received an incorrect packet, else
0
*/
...
sent from server, if any, by reinitializing, and catch up if needed
*/
void updateAudio();
