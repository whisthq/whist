...
void cacheFinders(JavaVM* vm) {
    ...
    jmethodID getClassLoaderMethod =
        (*env)->GetMethodID(env, fractalObjectClass, "getClassLoader", "()Ljava/lang/ClassLoader;");
    cachedClassLoader = (jobject)(*env)->NewGlobalRef(
        env, (*env)->CallObjectMethod(env, fractalClass, getClassLoaderMethod));
    cachedFindClassMethod = (*env)->GetMethodID(env, classLoaderClass, "findClass",
...
jclass findClass(const char* name) {
    JNIEnv* env = getEnv();
    return (jclass)((*env)->CallObjectMethod(env, cachedClassLoader, cachedFindClassMethod,
