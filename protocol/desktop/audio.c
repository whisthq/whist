...
void updateAudio() {
  ...
  while (still_more_audio_packets) {
    ...
    bool valid = true;
    for (int i = next_to_play_id; i < next_to_play_id + MAX_NUM_AUDIO_INDICES;
         i++) {
    ...
    if (valid) {
      ...
      if (SDL_GetQueuedAudioSize(AudioData.dev) > (unsigned int)real_limit) {
        ...
        encoded_packet.data =
            (uint8_t*)av_malloc(MAX_NUM_AUDIO_INDICES * MAX_PAYLOAD_SIZE);
        encoded_packet.size = 0;

        for (int i = next_to_play_id;
             i < next_to_play_id + MAX_NUM_AUDIO_INDICES; i++) {
          audio_packet* packet = &receiving_audio[i % RECV_AUDIO_BUFFER_SIZE];
          memcpy(encoded_packet.data + encoded_packet.size, packet->data,
                 packet->size);
