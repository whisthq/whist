 ...
 * @file ltr.c
 * @brief Implements the codec-independent decision logic for long-term references.
 */
...
static void ltr_dump_frame(LTRFrame *frame) {
    LOG("    { action { %d, %d }  fc %" PRIu64 "  ref %" PRIu64 "  id %" PRIu32
        " sent %d ack %d nack %d }",
        frame->action.frame_type, frame->action.long_term_frame_index, frame->frame_counter,
        frame->reference_frame_counter, frame->frame_id, frame->sent, frame->ack, frame->nack);
}
...
int ltr_pick_next_action(LTRState *ltr, LTRAction *action) {
    ...
    // If this is an intra frame then we don't want frames before it to
    // affect the stream after (though we do still want to track them).
...
static void ltr_ack_frame_internal(LTRState *ltr, LTRFrame *frame) {
    ...
    LOG("ACK for frame %d (action { %d, %d }, fc %" PRIu64 " ref %" PRIu64 ").", frame->frame_id,
        frame->action.frame_type, frame->action.long_term_frame_index, frame->frame_counter,
        frame->reference_frame_counter);
    frame->ack = true;

    if (frame->frame_counter >= ltr->intra_frame_counter) {
        // If this frame is a long-term reference then have a new good long
        // term reference to use.
        if (frame->action.frame_type == VIDEO_FRAME_TYPE_INTRA ||
            frame->action.frame_type == VIDEO_FRAME_TYPE_CREATE_LONG_TERM) {
            ltr->create_long_term_outstanding = false;
...
static void ltr_nack_frame_internal(LTRState *ltr, LTRFrame *frame, bool pending) {
    ...
    LOG("NACK for frame %d (action { %d, %d }, fc %" PRIu64 " ref %" PRIu64 ").", frame->frame_id,
        frame->action.frame_type, frame->action.long_term_frame_index, frame->frame_counter,
        frame->reference_frame_counter);
    frame->nack = true;
