...
static int memory_reconfigure_capture_device(MemoryCaptureDevice *dev, uint32_t width,
                                             uint32_t height, uint32_t dpi) {
    return 0;
}

static int memory_capture_device_init(MemoryCaptureDevice *dev, uint32_t width, uint32_t height,
                                      uint32_t dpi) {
...
static int memory_capture_screen(MemoryCaptureDevice *fc, uint32_t *width, uint32_t *height,
                                 uint32_t *pitch) {
...
static int memory_transfer_screen(MemoryCaptureDevice *fc, CUcontext *cuda_context) {
	*cuda_context = *get_video_thread_cuda_context_ptr();
	return 0;
}

static int memory_capture_getdata(MemoryCaptureDevice *device, void **buf, uint32_t *stride) {
    *buf = (void *)device->data_block;
    *stride = device->base.infos->pitch;
    return 0;
}

static int memory_get_dimensions(MemoryCaptureDevice *device, uint32_t *w, uint32_t *h,
                                 uint32_t *dpi) {
...
CaptureDeviceImpl *create_memory_capture_device(CaptureDeviceInfos *infos, MemoryCaptureParams* params) {
