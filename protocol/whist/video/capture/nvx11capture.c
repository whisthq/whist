...
typedef struct {
    ...
    volatile bool nvidia_context_is_stale;
...
static int nvx11_capture_device_init(NvX11CaptureDevice* device, uint32_t width, uint32_t height,
                                     uint32_t dpi) {
...
static int nvx11_capture_screen(NvX11CaptureDevice* device, uint32_t* width, uint32_t* height,
                                uint32_t* pitch) {
    ...
    if (!device->first_frame && device->switch_to_nvidia_device) {
        ...
        if (device->nvidia_context_is_stale) {
            CUcontext tmpCudaContext = NULL;
            CUresult cu_res = cu_ctx_pop_current_ptr(&tmpCudaContext);
            if (cu_res != CUDA_SUCCESS) {
                LOG_ERROR("Failed to pop current context, status=%d", cu_res);
            }
            LOG_DEBUG("popped last CUDA context %p", tmpCudaContext);
...
static int nvx11_get_dimensions(NvX11CaptureDevice* device, uint32_t* w, uint32_t* h,
                                uint32_t* dpi) {
    return device->x11impl->get_dimensions(device->x11impl, w, h, dpi);
}

static int nvx11_capture_getdata(NvX11CaptureDevice* device, void** buf, uint32_t* stride) {
...
static int nvx11_capture_transfer_screen(NvX11CaptureDevice* device, CUcontext* context) {
    int ret = 0;
    CUcontext tmp;
    CaptureDeviceImpl* impl = device->use_nvidia_device ? device->nvimpl : device->x11impl;
    if (impl && impl->transfer_screen) ret = impl->transfer_screen(impl, &tmp);

    if (device->use_nvidia_device && !device->first_frame)
        *context = device->nvidia_cuda_context;
    else
