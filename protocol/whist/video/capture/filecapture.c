...
static int file_reconfigure_capture_device(FileCaptureDevice *fc, uint32_t width, uint32_t height,
                                           uint32_t dpi) {
    int err;
...
static int file_capture_screen(FileCaptureDevice *fc, uint32_t *width, uint32_t *height,
                               uint32_t *pitch) {
    int err;
...
static int file_transfer_screen(FileCaptureDevice *fc) {
...
static int file_capture_getdata(FileCaptureDevice *device, void **buf, uint32_t *stride) {
    *buf = device->output_frame->data[0];
    *stride = device->output_frame->linesize[0];
    return 0;
}

static int file_get_dimensions(FileCaptureDevice *device, uint32_t *w, uint32_t *h, uint32_t *dpi) {
...
static int file_capture_device_init(FileCaptureDevice *dev, uint32_t width, uint32_t height,
                                    uint32_t dpi) {
...
CaptureDeviceImpl *create_file_capture_device(CaptureDeviceInfos *infos, const char *movie) {
    ...
    // impl->update_dimensions = (CaptureDeviceUpdateDimensionsFn)file_update_dimensions;
