...
static int x11_update_dimensions(X11CaptureDevice* device, uint32_t width, uint32_t height,
                                  uint32_t dpi) {
    /*
...
static int x11_reconfigure_capture_device(X11CaptureDevice* dev, uint32_t width, uint32_t height,
                                    uint32_t dpi) {
	...
	dev->segment.shmid = shmget(
		IPC_PRIVATE, dev->image->bytes_per_line * dev->image->height, IPC_CREAT | 0777);
...
static int x11_capture_device_init(X11CaptureDevice* dev, uint32_t width, uint32_t height, uint32_t dpi) {
    return x11_reconfigure_capture_device(dev, width, height, dpi);
}



static int x11_capture_screen(X11CaptureDevice* dev) {
    /*
    ...
    */
    ...
    if (accumulated_frames || dev->first) {
        ...
        } else if (infos->width != (uint32_t)window_attributes.width ||
        		infos->height != (uint32_t)window_attributes.height) {
            LOG_ERROR("Wrong width/height (infos=%dx%d window=%dx%d) !",
            		infos->width, infos->height, window_attributes.width, window_attributes.height);
            accumulated_frames = -1;
        } else {
...
static int x11_get_dimensions(X11CaptureDevice* device, uint32_t* w, uint32_t* h, uint32_t* dpi) {
    /*
...
static int x11_capture_getdata(X11CaptureDevice* device, void** buf, uint32_t* stride)
{
...
CaptureDeviceImpl* create_x11_capture_device(CaptureDeviceInfos* infos, const char *display) {
    ...
    if (!XDamageQueryExtension(dev->display, &damage_event, &damage_error))
    	goto out_damage_ext;

    dev->damage = XDamageCreate(dev->display, dev->root, XDamageReportRawRectangles);
    if (!dev->damage)
    	goto out_damage_create;
    ...
    // no impl->transfer_screen for X11
