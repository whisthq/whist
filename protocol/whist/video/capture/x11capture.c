...
static int x11_capture_device_init(X11CaptureDevice* dev, uint32_t width, uint32_t height,
                                   uint32_t dpi) {
    return x11_reconfigure_capture_device(dev, width, height, dpi);
}

static int x11_capture_screen(X11CaptureDevice* dev, uint32_t* width, uint32_t* height,
                              uint32_t* pitch) {
    /*
...
static int x11_capture_cuda_context(X11CaptureDevice* dev, CUcontext* context) {
...
CaptureDeviceImpl* create_x11_capture_device(CaptureDeviceInfos* infos, const char* display) {
    ...
    impl->cuda_context = (CaptureDeviceCudaContext)x11_capture_cuda_context;
