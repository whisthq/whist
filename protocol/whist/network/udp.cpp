...
int udp_get_num_pending_frames(SocketContext* socket_context, WhistPacketType type) {
    ...
    if (ring_buffer == NULL) {
        ...
        return get_num_pending_ready_frames(ring_buffer);
    }
}
        ...
        if (connection_timeout_ms == -1) {
            ...
            set_timeout(
                ...
                min(max(connection_timeout_ms - get_timer(&client_creation_timer) * MS_IN_SECOND,
                        0.0),
                    (double)CONNECTION_ATTEMPT_INTERVAL_MS));
        }
...
void udp_dedicated_recv_iterate(void* raw_context) {
    ...
    if (!got_packet) {
        // wake the processing thread after socket timeout, since we use wait_cond() instead of
        // timedwait_cond(). see also the comment on timedwait_cond() below
