...
DataRate LinkCapacityTracker::estimate() const {
  whist_plotter_insert("linkcap_tracker",-1, capacity_estimate_bps_/1000.0);
...
DataRate SendSideBandwidthEstimation::GetEstimatedLinkCapacity() const {
  //whist_plotter_insert("link_cap",-1, link_capacity_.estimate().kbps());
...
void SendSideBandwidthEstimation::UpdatePacketsLost(int64_t packets_lost,
  ...
  if (number_of_packets > 0) {
    ...
    whist_plotter_insert("ssbe::last_fraction_loss_", at_time.us()/1e6,last_fraction_loss_ / 256.0f);
  ...
  if (time_since_loss_packet_report < 1.2 * kMaxRtcpFeedbackInterval) {
    ...
    if (current_target_ < bitrate_threshold_ || loss <= low_loss_threshold_) {
      ...
      DataRate new_bitrate = DataRate::BitsPerSec(
          min_bitrate_history_.front().second.bps() * (1.f + cc_shared_state.k_increase_ratio) + 0.5);
#else
      DataRate new_bitrate = DataRate::BitsPerSec(
...
void SendSideBandwidthEstimation::UpdateTargetBitrate(DataRate new_bitrate,
  ...
  //fprintf(stderr,"bitrate=%f\n",double(new_bitrate.kbps()));
  whist_plotter_insert("loss-based_bitrate", at_time.us()/1e6, new_bitrate.kbps());
