...
typedef struct 
{
    ...
    int num_buffers_registered; //stores how many buffers we have "received" for this group, to help detect if decoding can happen in O(1) time 
}GroupInfo;
...
//if num of buffers is large then this, rs_wrapper will split the encoding/decoding into multiple groups
...
/*
below are two function are a very light innder wrapper of the rs lib, handle special case with dup and dedup with the same interface
*/
...
static RSWrapper * rs_wrapper_create_inner(int num_real_buffers, int num_total_buffers, int num_groups);

/*
the below 3 functions are highly associated, they decide how to partition the buffers, and how to do the mapping
*/
...
RSWrapper * rs_wrapper_create(int num_real_buffers, int num_total_buffers)
{
    ...
    int num_groups=int_div_roundup(num_total_buffers,rs_wrapper_max_group_size);  //inter div, round up
...
void rs_wrapper_encode(RSWrapper * rs_wrapper, void **src, void**dst, int sz)
{
    ...
    //if num of groups is one, we can simply pass the encode request to the underlying lib without maintain a lot of mappings
    ...
    for(int i=0;i<rs_wrapper->num_groups;i++)
    {
        ...
        for(int j=0;j<rs_wrapper->group_infos[i].num_fec_buffers;j++)
        {
            ...
            rs_encode_or_dup(k,n,src_sub,dst[full_index -rs_wrapper->num_real_buffers], sub_index, sz);
        }
    }
    free(src_sub);
}
...
int rs_wrapper_decode(RSWrapper * rs_wrapper, void **pkt, int * index, int num_pkt, int sz)
{
    ...
    //if num of groups is one, we can simply pass the decode request to the underlying lib without maintain a lot of mappings
     if(rs_wrapper->num_groups==1)
     {
         int k=rs_wrapper->group_infos[0].num_real_buffers;
         int n=k+rs_wrapper->group_infos[0].num_fec_buffers;
     ...
     for(int i=0;i<num_pkt;i++)
     {
         ...
         if(subgroup_offsets_cnt[info.group_id]== rs_wrapper->group_infos[info.group_id].num_real_buffers) continue; 
...
void rs_wrapper_decode_helper_register_index(RSWrapper * rs_wrapper,int index)
{
    ...
    if(rs_wrapper->group_infos[info.group_id].num_buffers_registered== rs_wrapper->group_infos[info.group_id].num_real_buffers)
    {
...
static void fill_partition_plan(RSWrapper * rs_wrapper)
{
    ...
    {
        rs_wrapper->group_infos[i].num_real_buffers= int_partition_helper(num_real_buffers,i,num_groups);
        rs_wrapper->group_infos[i].num_fec_buffers = int_partition_helper(num_fec_buffers,i,num_groups);
    }
}
...
static SubIndexInfo index_full_to_sub(RSWrapper *rs_wrapper,int index)
{
    ...
    else
    {
        ...
        info.sub_index=  rs_wrapper->group_infos[info.group_id].num_real_buffers+(index-rs_wrapper->num_real_buffers)/rs_wrapper->num_groups;
    }
    return info;
}
...
static int index_sub_to_full(RSWrapper *rs_wrapper,int group_id, int sub_index)
{
    ...
    {
        return rs_wrapper->num_real_buffers  +  group_id + (sub_index- rs_wrapper->group_infos[group_id].num_real_buffers) * rs_wrapper->num_groups;
    }
}
...
static RSWrapper * rs_wrapper_create_inner(int num_real_buffers, int num_total_buffers, int num_groups)
{
    ...
    for(int i=0;i<num_groups;i++)
    {
        ...
        rs_wrapper->group_max_num_real_buffers=max(rs_wrapper->group_max_num_real_buffers,rs_wrapper->group_infos[i].num_real_buffers);

        if(rs_wrapper->group_max_num_real_buffers!=1)
        {
            //warm up the rs_table, so that some computation happens at creation time, so that decode time can be faster.
            get_rs_code(rs_wrapper->group_max_num_real_buffers, rs_wrapper->group_max_num_real_buffers + rs_wrapper->group_infos[i].num_fec_buffers);
        }

        if(verbose_log)
        {
            //too spammy
            //fprintf(stderr,">>group_id=%d num_real_buffers=%d num_fec_buffers=%d\n", i,rs_wrapper->group_infos[i].num_real_buffers , rs_wrapper->group_infos[i].num_fec_buffers );
        }
    }
