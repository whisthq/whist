...
export const signupRequest = fromEventIPC('signupRequest').pipe(
  filter((req) => (((req?.email as string ?? '') !== '') && ((req?.password as string ?? '') !== ''))),
  switchMap((req) => from(createConfigToken().then(async (token) => await encryptConfigToken(token, req.password)).then((token) => [req, token]))),
