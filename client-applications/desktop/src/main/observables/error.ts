...
export const errorWindowRequest = merge(
  ...
  containerCreateFailure.pipe(
    map((response) => {
      if (containerCreateErrorNoAccess(response)) { return createContainerErrorWindowNoAccess }
      if (containerCreateErrorUnauthorized(response)) { return createContainerErrorWindowUnauthorized }
