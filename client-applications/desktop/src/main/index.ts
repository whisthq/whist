...
export const userLoginLoading = merge(
    userLoginRequests.pipe(mapTo({ loginLoading: true })),
    userLoginResponses.pipe(mapTo({ loginLoading: false }))
)
...
// loading state.
combineLatest([ipcStates, userLoginLoading], _.merge).subscribe((state: any) =>
    ipcBroadcast(state, getWindows())
)
