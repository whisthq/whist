...
import { identity, pick, has } from 'lodash'
...
interface DebugSchema {
  [title: string]: [string] | [string, null] | [string, (...args: any[]) => any]
}
...
// duplicates.
//
// When we decide to return to standard logging, we should wrap the block
// below with !app.isPackaged.
for (const key in symbols) {
  if (isObservable(symbols[key]) && has(schema, key)) {
