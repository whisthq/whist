...
const mandelboxFlow  = flow(
  ...
  (
    trigger: Observable<{ // flows take a single argument, which is always
      jwtIdentity: string         // a observable containing named parameters.
    ...
    const create = mandelboxCreateFlow(
      trigger.pipe(map((t) => pick(t, ["jwtIdentity", "accessToken", "region"])))
    ...
    const host = hostServiceFlow(
      zip([trigger, create.success]).pipe(
        map(([t, _c]) => pick(t, ["jwtIdentity", "accessToken", "configToken"]))
