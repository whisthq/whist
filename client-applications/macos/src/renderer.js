...
try {
    document.getElementById('username').value = typeof store.get('username') === 'undefined' ? '' : store.get('username')
    document.getElementById('password').value = typeof store.get('password') === 'undefined' ? '' : store.get('password')
    document.getElementById('remember-me').checked = typeof store.get('remember') === 'undefined' ? false : store.get('remember')
} catch(err) {}
...
document.querySelector('#login-button').addEventListener('click', () => {verifyUser()})
document.addEventListener('keypress', loginUser)

function verifyUser() {
    document.getElementById("warning").innerHTML = '<img src = "assets/spinner.svg" width = "14px" class = "fa-spin spinner" style = "margin-right: 5px"/>Verifying credentials'
...
function runProtocol(public_ip, tries) {
    ...
    var ffmpegpath = appRootDir + '/fractal-protocol/desktop/desktop'
    var parameters = [public_ip]
    // const spawn = require( 'child_process' ).spawn;
    // var child = spawn( ffmpegpath, [public_ip, 1024, 768]);  //add whatever switches you need here, test on command line first
    child(ffmpegpath, parameters, {detached: true, stdio: 'ignore'});
