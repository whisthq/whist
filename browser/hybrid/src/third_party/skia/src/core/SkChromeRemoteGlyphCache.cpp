...
public:
    ...
    SK_SPI bool transStrikeData(std::vector<uint8_t>* dst_memory, const void* src_memory, size_t memorySize,
                                std::map<SkDiscardableHandleId, SkDiscardableHandleId>& server_handle_map);
...
bool SkStrikeServerImpl::transStrikeData(std::vector<uint8_t>* dst_memory, const void* src_memory, size_t memorySize,
                                         std::map<SkDiscardableHandleId, SkDiscardableHandleId>& server_handle_map) {
    SkASSERT(memorySize != 0u);
    Deserializer deserializer(static_cast<const volatile char*>(src_memory), memorySize);
    Serializer serializer(dst_memory);
    ...
    serializer.emplace<uint64_t>(SkTo<uint64_t>(strikeCount));

    for (size_t i = 0; i < strikeCount; ++i) {
        StrikeSpec spec;
        if (!deserializer.read<StrikeSpec>(&spec)) READ_FAILURE
        serializer.emplace<StrikeSpec>(spec.fTypefaceID, server_handle_map[spec.fDiscardableHandleId]);
        SkAutoDescriptor ad;
        if (!deserializer.readDescriptor(&ad)) READ_FAILURE
        serializer.writeDescriptor(*ad.getDesc());
        #if defined(SK_TRACE_GLYPH_RUN_PROCESS)
        ...
        serializer.emplace<bool>(fontMetricsInitialized);
        ...
        serializer.emplace<uint64_t>(glyphImagesCount);
        for (size_t j = 0; j < glyphImagesCount; j++) {
            SkTLazy<SkGlyph> glyph;
            if (!SkStrikeClientImpl::ReadGlyph(glyph, &deserializer)) READ_FAILURE
            write_glyph(*glyph, &serializer);
            ...
            if (!glyph->isEmpty() && SkStrikeForGPU::FitsInAtlas(*glyph)) {
                ...
                memcpy(serializer.allocate(glyph->imageSize(), glyph->formatAlignment()),
                       (const void *)image, glyph->imageSize());
            }
        }

        if (!deserializer.read<uint64_t>(&glyphPathsCount)) READ_FAILURE
        serializer.emplace<uint64_t>(glyphPathsCount);
        for (size_t j = 0; j < glyphPathsCount; j++) {
            ...
            if (pathSize > 0) {
                ...
                memcpy(serializer.allocate(pathSize, kPathAlignment),
                       (const void *)pathData, pathSize);

                pathPtr = &path;
                if (!deserializer.read<bool>(&hairline)) READ_FAILURE
                serializer.write<bool>(hairline);

            }
        }

        if (!deserializer.read<uint64_t>(&glyphDrawablesCount)) READ_FAILURE
        serializer.emplace<uint64_t>(glyphDrawablesCount);
        ...
        for (size_t j = 0; j < glyphDrawablesCount; j++) {
            ...
            serializer.write<uint64_t>(drawableSize);

            if (drawableSize > 0) {
                auto* drawableData = deserializer.read(drawableSize, kDrawableAlignment);
                if (!drawableData) READ_FAILURE
                memcpy(serializer.allocate(drawableSize, kDrawableAlignment),
                       (const void *)drawableData, drawableSize);
            }
        }
    }
...
bool SkStrikeServer::transStrikeData(std::vector<uint8_t>* dst_memory,
                                     const void* src_memory, size_t memorySize,
                                     std::map<SkDiscardableHandleId, SkDiscardableHandleId>& server_handle_map) {
    return fImpl->transStrikeData(dst_memory, src_memory, memorySize, server_handle_map);
}
