...
```bash
pg_dump --no-owner --no-privileges --schema-only $DB_URL > backend/webserver/db_migration/schema.sql
```

## High-Level Overview

The Whist migration strategy revolves around treating our database schema as code. Like anything else in the codebase, this involves checking a schema "source of truth" into version control. This takes the form of a file in `whist/backend/webserver/db-migration` called `schema.sql`.
...
Any change to `whist/backend/webserver` will trigger the migration workflow.
