...
def logs_contain_errors(logs_root_dir, verbose=False):
    ...
    client_log_file = os.path.join(logs_root_dir, "client", "protocol_client-out.log")
    server_log_file = os.path.join(logs_root_dir, "server", "protocol_server-out.log")
...
def download_latest_logs(
    ...
    compared_client_log_path = os.path.join(".", branch_name, "client", "protocol_client-out.log")
    compared_server_log_path = os.path.join(".", branch_name, "server", "protocol_server-out.log")
    ...
    for folder_name in reversed(folders):
        ...
        for obj in bucket.objects.filter(Prefix=f"{branch_name}/{subfolder_name}"):
            if "protocol_client-out.log" in obj.key:
                bucket.download_file(obj.key, compared_client_log_path)
            elif "protocol_server-out.log" in obj.key:
            ...
            elif "plot_data.json" in obj.key:
                bucket.download_file(
                    obj.key,
                    compared_client_plot_data_path
                    if "client/plot_data.json" in obj.key
                    else compared_server_plot_data_path,
                )
