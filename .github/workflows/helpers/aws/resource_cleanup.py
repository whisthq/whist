...
def get_all_aws_asgs(region):
    ...
    all_asgs = [asg["AutoScalingGroupARN"] for asg in json.loads(all_asgs)["AutoScalingGroups"]]
    return all_asgs
...
def get_all_aws_clusters(region):
    all_cluster_arns, _ = subprocess.Popen(
        ["aws", "ecs", "list-clusters", "--no-paginate", "--region", region], stdout=subprocess.PIPE
    ).communicate()
...
def cluster_to_asgs(cluster_arn, region):
    ...
    capacity_providers = json.loads(capacity_providers)["clusters"][0]["capacityProviders"]
...
def get_hanging_clusters(urls, secrets, region):
    # parses names of clusters from ARNs
    aws_clusters = set([cluster.split("/")[-1] for cluster in get_all_aws_clusters(region)])
