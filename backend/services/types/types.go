...
type (
	...
	// CloudProvider is the name of a cloud provider (e.g. AWS, GCP), but it is added as a custom type to
...
// TODO: Candidate functions for using generics
func (p CloudProvider) String() string {
	return string(p)
}

func (pr PlacementRegion) String() string {
	return string(pr)
}
