...
func TestUserConfigChecksum(t *testing.T) {
	...
	_, err = configutils.UploadFileToBucket(s3Client, configutils.GetAccessPoint(), "user_config_test_user/checksum_test", testFile1)
	if err != nil {
		t.Fatalf("failed to upload test file: %v", err)
	}

	// Check that checksum is present and correct
	head, err := configutils.GetHeadObject(s3Client, configutils.GetAccessPoint(), "user_config_test_user/checksum_test")
	if err != nil {
