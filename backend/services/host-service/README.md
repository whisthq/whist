...
1. Start your instance, ssh into it and go to `~/whist/backend/services/host-service`, then run `make run`.
...
1. Start your instance, ssh into it and go to `~/whist/backend/services/host-service`, then run `make run` and wait for the event loop to start. Take note of your dev instance ip address.
