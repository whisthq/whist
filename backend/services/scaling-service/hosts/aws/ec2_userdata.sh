...
# Changes:
#   - changed blocksize to 1M from 128k because optimal dd blocksize is 1M
#   - TODO: make host fs read-only via /etc/fstab so we can compress the host even more, and don't have to worry about warming up writes
...
#   - TODO: make the AMI as small as possible as well
#   - TODO: experiment with iodepth, more fio workers, etc. (`offset_increment` with multiple jobs)
