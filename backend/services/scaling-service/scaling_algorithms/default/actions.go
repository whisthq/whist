...
func (s *DefaultScalingAlgorithm) VerifyCapacity(scalingCtx context.Context, event ScalingEvent) error {
	...
	if mandelboxCapacity < desiredFreeMandelboxes {
		logger.Infof("Current mandelbox capacity of %v is less than desired %v. Scaling up %v instances to satisfy minimum desired capacity.", mandelboxCapacity, desiredFreeMandelboxes, defaultInstanceBuffer)
		...
		logger.Infof("Mandelbox capacity %v in %v is enough to satisfy minimum desired capacity of %v.", mandelboxCapacity, event.Region, desiredFreeMandelboxes)
	}
...
func (s *DefaultScalingAlgorithm) ScaleDownIfNecessary(scalingCtx context.Context, event ScalingEvent) error {
	...
	extraCapacity := desiredFreeMandelboxes + (defaultInstanceBuffer * instanceCapacity["g4dn.2xlarge"])
