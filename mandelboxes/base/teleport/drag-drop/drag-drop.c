...
typedef struct FileTransferContext {
    char id[NAME_MAX + 1];        // Every file transfer will have a unique ID, which we
                                  // store here as a string.
    char id_path[NAME_MAX + 1];   // `/home/whist/.teleport/drag-drop/downloads/[ID]`. We must
                                  // keep this allocated for the inotify thread to work.
    bool data_ready;              // `true` if the protocol has finished dumping the file, else
                                  // `false`.
    bool active;                  // `true` if the protocol has started dumping the file, else
                                  // `false`.
    char filename[NAME_MAX + 1];  // The name of the file that the protocol is dumping.
    int create_wd;                // The inotify watch descriptor keeping track of `id_path` for
                                  // FS events indicating that the file was created
    int file_size;                // The size of the file, in bytes
} FileTransferContext;

// We just die after 1024 transfers, not worth being robust to this for the time
// being.
...
void wait_for_transfer_context_ready(int idx) {
    struct timespec ts = {.tv_sec = 0, .tv_nsec = 1000 * 500};
...
typedef struct DirectoryTreeResult {
    ...
    int transfer_context_idx;  // which transfer context, if any, the path
                               // corresponds to
} DirectoryTreeResult;
...
static void *teleport_fuse_init(struct fuse_conn_info *conn, struct fuse_config *cfg) {
    (void)conn;
    // Once a file is read, subsequent reads can be made from a kernel-level
    // buffer cache
    cfg->kernel_cache = 1;
    return NULL;
}
...
static int teleport_fuse_getattr(const char *path, struct stat *stbuf, struct fuse_file_info *fi) {
    (void)fi;
    int res = 0;
    ...
    switch (dt_res.node_type) {
        ...
        case NODE_TYPE_FILE:
            ...
            stbuf->st_nlink = 1;
            // Get file size. If -1, then set to default of 50
            if ((stbuf->st_size = transfer_status[dt_res.transfer_context_idx].file_size) == -1) {
                stbuf->st_size = 50;
            }
            break;
        default:
...
static int teleport_fuse_readdir(const char *path, void *buf, fuse_fill_dir_t filler, off_t offset,
    ...
    switch (dt_res.node_type) {
        ...
        case NODE_TYPE_ID:
            if (transfer_status[dt_res.transfer_context_idx].active) {
                ...
                st.st_nlink = 1;
                // Get file size. If -1, then set to default of 50
                if ((st.st_size = transfer_status[dt_res.transfer_context_idx].file_size) == -1) {
                    st.st_size = 50;
                }
...
// For a given file path, allow for read-only opening and cache the file
// descriptor in a FUSE-internal cache.
...
// For a given file path, close the underlying file descriptor when all
// references to the file handle are closed.
...
// let's assume we don't get more than 50 events at a time; we'll drop anything
// more
#define INOTIFY_BUFFER_SIZE (50 * INOTIFY_EVENT_SIZE)
// the file descriptor which inotify uses to coordinate and perform its
// operations
int inotify_fd;
...
void inotify_handle_file_create(int create_wd, const char *filename) {
    ...
    if (transfer_status[idx].active) {
        printf("ID <%s> already associated with file <%s>; skipping file <%s>\n",
               transfer_status[idx].id, transfer_status[idx].filename, filename);
        return;
    }
...
void inotify_handle_new_id(const char *id) {
    ...
    if (current_idx < MAX_TRANSFERS) {
        ...
        if (file_size_file) {
            ...
            fclose(file_size_file);
        } else {
            transfer_status[current_idx].file_size = -1;
        }
        ...
        // Right after creating a watch, we need to manually check for it to avoid
        // race conditions
...
// Updates the global state of `transfer_status` according to folders and files
// written in `/home/whist/.teleport/drag-drop/downloads`.

/**
 * @brief                  Update the global state of `transfer_status`
 * according to folders and files written in the directory
 *                          `/home/whist/.teleport/drag-drop/downloads`.
 *
 * @param opaque           Unused. Simply exists to match the `pthread_create`
 * signature.
 *
 * @return                 Unused. Simply exists to match the `pthread_create`
 * signature.
 */
void *multithreaded_download_watcher(void *opaque) {
    ...
    int id_wd =
        inotify_add_watch(inotify_fd, WHIST_TELEPORT_DRAG_DROP_DIRECTORY "/downloads", IN_CREATE);

    // watch descriptor for /ready/[id] files
    int ready_wd =
        inotify_add_watch(inotify_fd, WHIST_TELEPORT_DRAG_DROP_DIRECTORY "/ready", IN_CREATE);

    // Right after adding a watch, we need to also manually check for it to
    // appropriately handle any race condition.
...
int main(int argc, char *argv[]) {
    ...
    // We have to unshare before creating the pthread, because apparently doing
    // things in the other order is liable to cause really funky behavior.
    unshare_thread();
