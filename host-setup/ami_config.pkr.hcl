...
source "amazon-ebs" "Whist_AWS_AMI_Builder" {
  ...
  launch_block_device_mappings {
    ...
    volume_size           = 6    # GB, assumes we use g4dn.2xlarge (2 mandelboxes maximum) - as small as possible to save on cost/warmup time, but large-enough to run Ubuntu + the mandelboxes. In theory, below this size, warmup time is not reduced because disk performance is throttled.
