...
This script takes a blank Ubuntu 20.04 GPU-accelerated virtual machine (AWS or Azure)
and sets it up for running Whist by configuring Docker and NVIDIA to run Whist's GPU-enabled
mandelboxes.
...
EOF
  ...
  ./get-nvidia-driver-installer.sh "$CLOUD_PROVIDER"
...
local_development_steps () {
  ...
  # Here we symlink all cloud provider folders, regardless of which one we're 
  # running on, since they can all exist concurrently and this avoids passing 
  # an argument to this function.
  sudo ln -sf /home/ubuntu/.aws /root/.aws
  sudo ln -sf /home/ubuntu/.azure /root/.azure
}
...
if [[ -n "$LOCAL_DEVELOPMENT" ]]; then
  ...
  # On local development, we only send the cloud provider as an argument
  CLOUD_PROVIDER=${1}
