...
# 1. hardware.region_to_ami
# 2. hardware.supported_app_images
...
if [[ ^$POSTGRES_URI =~ "postgres://" ]]; then
    ...
    (pg_dump -d $POSTGRES_URI --data-only --column-inserts -t hardware.region_to_ami -t hardware.supported_app_images) > db_data.sql
