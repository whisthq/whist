...
def test_empty_instances(region_name: str) -> None:
    ...
    """
    assert find_instance(region_name, CLIENT_COMMIT_HASH_FOR_TESTING) is MandelboxAssignError.NO_INSTANCE_AVAILABLE
...
def test_assignment_logic(bulk_instance: Callable[..., InstanceInfo], location: str) -> None:
    ...
    assert (
        find_instance(location, CLIENT_COMMIT_HASH_FOR_TESTING) is MandelboxAssignError.NO_INSTANCE_AVAILABLE
