...
const post = configPost({ server: `https://${config.auth0.AUTH_DOMAIN_URL}` })
  ...
  */
  ...
  return await post({
    ...
    body: {
      ...
      client_id: config.auth0.AUTH_CLIENT_ID,
      code,
      redirect_uri: config.urls.CLIENT_CALLBACK_URL,
    },
  })
}
...
export const authPortalURL = () =>
  [
    `https://${config.auth0.AUTH_DOMAIN_URL}/authorize`,
    `?audience=${config.auth0.AUTH_API_IDENTIFIER}`,
