...
const fetchTeamData = async () => {
  await notion.databases.query({
  ...
  }).then(db => Promise.all(db.results.map(async row => {
    ...
    await request({
      ...
      const filepath = path.join("public", NOTION_DATA_DIRECTORY, `${row.id}.png`)
      fs.writeFileSync(filepath, body)
    })

    return {
      name: pageContents?.properties?.Name?.title?.[0]?.plain_text ?? "", // e.g. Marcus Aurelius
      title: pageContents?.properties?.Title?.rich_text?.[0]?.plain_text ?? "", // e.g. Roman emperor
...
const fetchInvestorData = async () => {
  await notion.databases.query({
    ...
    return {
      name: pageContents?.properties?.Name?.title?.[0]?.title?.plain_text ?? "", // e.g. Sam Lessin
      website: pageContents?.properties?.Website?.url ?? "", // e.g. http://slow.co
      description: pageContents?.properties?.Description?.rich_text?.[0]?.plain_text ?? "", // e.g. "Great Investor"
      imagePath: path.posix.join(NOTION_DATA_DIRECTORY, `${row.id}.png`), // e.g. assets/company/{row.id}.png
    }
  }))).then(team => {
    const jsonTarget = path.join("public", NOTION_DATA_DIRECTORY, "investors.json")
    fs.writeFileSync(jsonTarget, JSON.stringify(team))
  })
}
...
module.exports = {
  fetchCompanyInfo: async () => {
    fs.rmSync(path.join("public", NOTION_DATA_DIRECTORY), { recursive: true, force: true })
