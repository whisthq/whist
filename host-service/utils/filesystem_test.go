...
// TestWriteToNewFileButFileExists  should overwrite the file the second time trying to write to new file
func TestWriteToNewTestWriteToNewFileButFileExistsFile(t *testing.T) {
	// Create directory
	destDir, err := ioutil.TempDir("", "testWriteToNewFile")
	if err != nil {
		t.Fatal(err)
	...
	if err := WriteToNewFile(filePath, testFileContent); err != nil {
		t.Fatalf("error writing to file that already exists %s. Expected nil, got %v", filePath, err)
	}
